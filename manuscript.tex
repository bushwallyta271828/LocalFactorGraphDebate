...
\maketitle
\begin{center}
\fbox{\parbox{.92\textwidth}{\textit{All text in this document is
AI-generated. For a human-vetted version of this material, see the original
LessWrong post.}}}
\end{center}
\begin{abstract}
...
result to the answer from the whole model? A small set of arguments first
gives a strategic guarantee. Message passing then bounds the influence of everything
...
A random interaction table produces both an interaction and random local
fields, as \eqref{eq:contrast} makes explicit. A strong local field reduces
the slope of the edge response \eqref{eq:slope}, so increasing the scale of
the tables need not make debate harder; Section~\ref{sec:ensemble} makes this
quantitative at degrees three and four. Completion stability is also not the
same as weak spin correlation: with zero fields and pure interactions
$J_ex_ux_v/2$, global spin-flip symmetry gives $L_S=0$ for every induced
submodel. All core oscillations then vanish, even when strong positive
interactions make spins highly correlated.

For debate as a method of supervision, see \cite{irving,browncohen}.
...
\caption{Integer debate budgets for target mean absolute error $0.1$ on
$50{,}000$-vertex partially filled regular trees, matching the core panel.
The $240$ resolved cells retain simultaneous $95\%$ confidence decisions,
conditional on valid guarded numerical enclosures. Six boundary cells are
assigned approximately: choose the smaller budget when its sample mean-error
upper bound is at most $0.101$, otherwise the larger budget. Hatching marks
the cutoff $h\ge10$ under this convention. The strict statistical brackets
are retained separately in the figure data.}
\end{figure}
\begin{figure}[htbp]
\centering
...
\subsection{An explicit lower bound}
...
The remaining difficulty is estimating how stable a small core can be. On
the boards of Figure~\ref{fig:paired}, the mean optimized-core oscillation
at $h=100$ is about $0.024$, while the uniform bound of
Theorem~\ref{thm:upper} evaluates to about $0.23$ there: a core chosen after
seeing the tables can be much better than the radius bound predicts.

The lower bound concerns the induced-submodel judge and the class $\Vcl_k$
of Section~\ref{sec:lower}; it is not an impossibility theorem for other
judges or protocols. Likewise $\rho<1$ is a sufficient condition. Neither a
gray heatmap cell nor a run stopped at a budget cutoff shows that debate
fails there.
...
a population crossing on the same $50{,}000$-vertex ensemble using guarded
numerical enclosures and sequential statistical tests; six boundary colors
add the stated approximate $1\%$ mean-error tolerance. The ensemble panel evaluates
a size-uniform theorem. The lower-exponent panel evaluates a different
metric, RMS, with depth increasing logarithmically in the budget.
...
they are not simultaneous confidence bands over every budget.
